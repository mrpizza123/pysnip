...
class FeatureConnection(ServerConnection):
    ...
    def on_login(self, name):
        ...
        self.protocol.irc_say('* %s (IP %s) entered the game' % (self.name, self.address[0]))
    ...
    def on_disconnect(self):
        if self.name is not None:
            ...
            self.protocol.irc_say('* %s (IP %s) disconnected' % (self.name, self.address[0]))
    ...
    def kick(self, reason = None, silent = False):
	    ...
	    self.protocol.irc_say('%s has been kicked by %s : %s ' % (self.name, connection.name, reason))
